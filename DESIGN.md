...
- **Share sheet**: intent filters for `ACTION_SEND` and `ACTION_SEND_MULTIPLE` (`*/*`). `ShareActivity` is a near-invisible trampoline: secure the sources, enqueue work, show a "queued ✓" confirmation, finish. *Securing* means `takePersistableUriPermission` where the sender offered a persistable grant (the in-app picker always does), and otherwise a private copy of the bytes taken before the activity finishes — a plain `ACTION_SEND` grant is revoked with the receiving activity, so a queued upload would have nothing left to read on its first retry.
  ...
  **As of 2026-08-24 the trampoline reads only `EXTRA_STREAM`** (`ShareActivity.sharedUris`), which means a *text* share — select a line in a note app, tap Share, choose **table** — matches the `*/*` filter, reaches the activity, finds no stream, and is turned away with *"Nothing to put on the table."* Table offers itself for the job and then says there was nothing there, which is worse than not appearing at all. The filter is not the thing to narrow: this is `../DESIGN.md` §3 rule 16's ladder missing its text rung. The fix is to read `EXTRA_TEXT` and stage the bytes — with `EXTRA_STREAM` still winning when a sender supplies both, because rung 1 precedes rung 3 and a photo shared with a caption is a photo. `EXTRA_SUBJECT`, where the sender offers one, is a better name than the text's own first line (a browser sends the page title there and the URL as the text). PROGRESS C8.

  Note for whoever picks this up: **no new machinery is needed.** `UploadStaging.stage` already takes any `InputStream`, `stagedSourceUri` already makes a `file:` URI, `TransferQueue.upload` already takes a plain `String` URI with a name and a size, and `safeDisplayName` already exists in `transfer/DisplayNames.kt`. Only `UploadIntake`'s public entry is `Uri`-typed; everything under it is already byte-oriented.
...
5. The intake ladder behind all four entry points (`../DESIGN.md` §3 rules 16–23): the share-text rung first, because it also clears a live dead end (§4); then paste and the image rung.
