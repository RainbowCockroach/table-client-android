...
3. `../UI.md` (workspace root) — the shared client UI: layout, row anatomy, icons, state strings, settings, colour, and §11 shape/depth/ground. Its §12 is a UI conformance checklist, the surface-level twin of §3 above; §13 records what this client still has to change. Read it before touching any screen.
...
- KDoc on public API only, one line unless the behavior is genuinely subtle.
- No commented-out code, no section banners, no TODOs that don't name a concrete follow-up (ideally a `PROGRESS.md` checkpoint).
