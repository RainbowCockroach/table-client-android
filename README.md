...
```sh
# dev server: short TTL so the expiry test runs, fault injection for checkpoint C2
...
TABLE_URL=http://127.0.0.1:8080 TABLE_API_KEY=devkey TABLE_TTL_SECONDS=5 \
...
`TABLE_TTL_SECONDS` gates the expiry test alone.
